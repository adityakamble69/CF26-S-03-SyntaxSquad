      ...
      <div className="rounded-lg border border-navy/10 dark:border-white/10 bg-white dark:bg-[#132230] p-5">
        ...
        {metrics && (
          <>
            ...
            <div className="mt-4 pt-4 border-t border-navy/10 dark:border-white/10 flex flex-col gap-2">
              {!baselineName ? (
                <button
                  type="button"
                  onClick={onPinBaseline}
                  disabled={isRunning}
                  className="w-full inline-flex items-center justify-center gap-1.5 rounded-md border border-dashed border-navy/30 dark:border-white/30 px-3 py-2 text-xs font-semibold text-navy dark:text-white bg-transparent hover:bg-surface dark:hover:bg-navy/40 transition-colors disabled:opacity-50"
                >
                  Pin current run as baseline
                </button>
              ) : (
                <div className="flex items-center justify-between gap-2 rounded bg-surface dark:bg-[#0B131C] dark:border dark:border-white/10 p-2.5 text-xs">
                  <span className="truncate text-navy dark:text-white font-medium">
                    📍 Pinned: {baselineName}
                  </span>
                  <button
                    type="button"
                    onClick={onClearBaseline}
                    className="text-[10px] font-bold uppercase tracking-wider text-critical hover:underline"
                  >
                    Clear
                  </button>
                </div>
